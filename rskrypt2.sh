...
while true; do
    ...
    case $choice in
        ...
        2) read -p "Podaj katalog (Enter = /): " directory
           if test "$directory" = ""; then
               directory="/"
           fi ;;
        3) read -p "Podaj pierwszą opcję (np. -size +1M): " option1 ;;
        4) read -p "Podaj drugą opcję (np. -mtime -3): " option2 ;;
        5) read -p "Podaj frazę do wyszukania w pliku: " content ;;
        6)
            echo "Wyszukiwanie..."

            # Budujemy komendę find
            find_cmd="find \"$directory\" -type f"

            if test "$file_name" != ""; then
                find_cmd="$find_cmd -name \"*$file_name*\""
            fi
            if test "$option1" != ""; then
                find_cmd="$find_cmd $option1"
            fi
            if test "$option2" != ""; then
                find_cmd="$find_cmd $option2"
            fi
